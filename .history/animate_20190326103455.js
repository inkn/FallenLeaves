...
function falling() {
    ...
    for (let i = 0; i < leaves.length; i++) {
        ...
        vertices.rotation.y = 0.4*time * (i < 4 ? i + 1 : -(i + 1));
        vertices.geometry.vertices.forEach(v => {
            v.y -= Math.random()*5*0.1 -0.1
            v.x -= Math.random()*5*0.05 -0.08
            v.z -= Math.random()*5*0.05 -0.08
            if(v.y< -10) v.y = Math.random() * 300 + 400
            if(v.x< -500) v.x = Math.random() * 300 - 150
            if(v.z< -500) v.z = Math.random() * 300 - 150
        })
