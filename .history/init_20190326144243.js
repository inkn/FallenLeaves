...
function initStats() {
    ...
    stats = new Stats()
    document.getElementById('plugins').appendChild(stats.dom)
...
function initControlsManyou() {
    ...
    document.getElementById('main').addEventListener('keyup', function (e) {
        if (e.keyCode === 16) {
            speed = 5
        }
    ...
    document.getElementById('main').addEventListener('mousedown', function (e) {
        ...
        if (controlType === '漫游模式' && e.currentTarget === this) {
            controls.lock();
